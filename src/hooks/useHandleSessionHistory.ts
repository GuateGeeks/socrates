...
export function useHandleSessionHistory() {
  ...
  const extractMessageText = (content: any[] = []): string => {
    ...
    return content
      .map((c) => {
        ...
        if (c.type === "audio" || c.type === "input_audio" || c.type === "output_audio") {
          return c.transcript ?? "";
        }
        return "";
      })
  ...
  function handleAgentToolStart(
    ...
    _agent: any,
    functionTool: any,
    metadata?: { toolCall?: any },
  ) {
    const functionCall = metadata?.toolCall;
    const function_name = functionCall?.name ?? functionTool?.name;
    const function_args = maybeParseJson(functionCall?.arguments);
  ...
  function handleAgentToolEnd(
    ...
    const function_name = metadata?.toolCall?.name ?? functionTool?.name;
    addTranscriptBreadcrumb(
  ...
  function handleHistoryAdded(item: any) {
    ...
    if (itemId && role) {
      ...
      // If the guardrail has been tripped, this message gets sent to the assistant to correct it,
      // so we add it as a breadcrumb instead of a message.
      const guardrailMessage = sketchilyDetectGuardrailMessage(text);
      if (guardrailMessage) {
        const failureDetails = maybeParseJson(guardrailMessage);
        addTranscriptBreadcrumb("Guardia de Salida Activa", { details: failureDetails });
      } else {
